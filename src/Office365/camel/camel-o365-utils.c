	...
	for (ii = 0; ii < len; ii++) {
		...
		if (camel_name_value_array_get (headers, ii, &name, &value) && name && *name && value &&
			...
			if (g_ascii_strncasecmp (name, "X-", 2) == 0) {
				...
				/* Preserve only the first five... (see the comment at o365_folder_append_message_sync()) */
				if (did_add < 5)
				...
				GCancellable *cancellable)
{
	...
	o365_utils_add_file_attachment_content (builder, dw, ADD_ATTACHMENT_WITH_CONTENT_TYPE | ADD_ATTACHMENT_DECODE_CONTENT, cancellable);
					...
					GCancellable *cancellable)
{
	...
	o365_utils_add_file_attachment_content (builder, dw, ADD_ATTACHMENT_PREFIX_CONTENT_TYPE_HEADER, cancellable);
}
			 ...
			 GCancellable *cancellable)
{
	...
	for (ii = 0; ii < nn; ii++) {
		...
		if (CAMEL_IS_MULTIPART (dw)) {
			if (!o365_utils_do_multipart (CAMEL_MULTIPART (dw), is_first, out_body_part, out_attachments, cancellable))
				return FALSE;
			continue;
		}
		...
		*out_attachments = g_slist_prepend (*out_attachments, g_object_ref (message));
		...
		if (CAMEL_IS_MULTIPART (dw)) {
			...
			if (CAMEL_IS_MULTIPART_SIGNED (multipart) && camel_multipart_get_number (multipart) == 2) {
				o365_utils_do_smime_signed (multipart, &body_part, out_attachments, cancellable);
			} else {
				o365_utils_do_multipart (multipart, &is_first, &body_part, out_attachments, cancellable);
			}
		} else if (dw) {
			CamelContentType *type;
	...
	if (body_part) {
		...
		if (o365_content_type != E_O365_ITEM_BODY_CONTENT_TYPE_UNKNOWN) {
			...
			if (mem && wrote >= 0) {
				...
				e_o365_mail_message_add_body (builder, o365_content_type, (const gchar *) byte_array->data);
			}
